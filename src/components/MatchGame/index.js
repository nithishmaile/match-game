...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.uniqueId = setInterval(this.countdown, 1000)
  }
  ...
  onClickPlayAgain = () => {
    this.setState({isGameRunning: true, seconds: 60, score: 0})
  ...
  getId = id => {
    ...
    this.setState({randomObject: imagesList[randomIndex]})
    this.isCorrectImage(id)
  }
  ...
  isCorrectImage = id => {
    ...
    const isCorrectOrNot = id === randomObject.id
    if (isCorrectOrNot === true) {
      this.setState(preVState => ({score: preVState.score + 1}))
    } else {
      this.setState({isGameRunning: false}, clearInterval(this.uniqueId))
    }
  }
          ...
          <nav className="nav-container">
            ...
            <ul className="score-container">
              ...
              <li className="score-div">
                ...
                />
                <p className="span-element">{seconds} sec</p>
              </li>
            </ul>
          </nav>
          <div className="bottom-section">
            {isGameRunning ? (
              <div className="game-container">
                <img src={randomImage} alt="match" className="main-image" />
                <ul className="unordered-list">
                  {tabsList.map(eachObj => (
                    <TabItems
                      tabsListDetails={eachObj}
                      key={eachObj.tabId}
                      getTabCategory={this.changeActiveId}
                      isActive={eachObj.tabId === activeTabId}
                    />
                  ))}
                </ul>
                <u1 className="thumbnails-unOrderedList">
                  {filteredResults.map(eachObj => (
                    <CardItems
                      key={eachObj.id}
                      thumbnailsList={eachObj}
                      getId={this.getId}
                    />
                  ))}
                </u1>
              ...
              <div className="gameOver-container">
                ...
                />
                <p className="yourScore-heading">Your Score</p>
                <h1 className="last-score">{score}</h1>
                <button
                  type="button"
                ...
                >
                  <img
                    ...
                    alt="reset"
                  />
                  <p className="button-heading">PLAY AGAIN</p>
                </button>
              </div>
            )}
          </div>
